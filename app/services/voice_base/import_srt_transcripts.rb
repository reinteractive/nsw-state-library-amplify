...
module VoiceBase
  ...
  class ImportSrtTranscripts
    ...
    def call
      ...
      Rails.logger.debug(
        "Retrieved #{transcripts.count} empty transcripts from collections with VoiceBase as their vendor."
      )
    ...
    def read_lines_from_file(transcript)
      if transcript.script.file.nil?
        ...
        unless File.exist?(fp)
          Rails.logger.debug("Couldn't find transcript in project folder for Transcript ##{transcript.id} at #{fp}.")
          return []
        end
        File.read(fp).lines
      else
